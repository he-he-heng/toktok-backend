...
func (rq *RelationQuery) QueryMessages() *MessageQuery {
...
func (rq *RelationQuery) WithMessages(opts ...func(*MessageQuery)) *RelationQuery {
	query := (&MessageClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RelationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Relation, error) {
	...
	if query := rq.withMessages; query != nil {
		if err := rq.loadMessages(ctx, query, nodes,
			func(n *Relation) { n.Edges.Messages = []*Message{} },
			func(n *Relation, e *Message) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RelationQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Relation, init func(*Relation), assign func(*Relation, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(relation.MessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.relation_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "relation_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "relation_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
